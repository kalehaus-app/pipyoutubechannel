...
# V3 — full fast-cut pass (no generation credits)

Applies the opening experiment's finding to the whole episode. **No images or
videos were generated.** V1 and V2 remain untouched.

| | |
|---|---|
| **V3 master** | `a27c2c1f-a364-4fa7-ae8a-b5ea0e5212a5` — 1920x1080, 305.751 s, 255.4 MB |
| **V3 preview** | `c361ed59-22da-4c44-9329-6922273be49d` — 854x480, 305.750 s, 22.2 MB |
| **Shots** | 95 (V2 had ~30) |
| **Average shot** | **3.22 s** (V2 ~10 s) |
| **Credits spent** | 0 |

## Method

Rebuilt from the V2 master, so the story, song sync and total length are
identical. The change is purely how it is cut: the timeline is walked in short
segments that alternate between the **wide frame** and a **1.28x punch-in**
(centre, left-biased, right-biased on rotation, never two punches in a row).

Continuous action with a jumping frame scale reads as multi-camera coverage.
That is what raises the cut rate without needing more footage — the constraint
that makes a naive fast cut impossible, since racing through the existing clips
would exhaust the story in about 90 seconds and leave three minutes empty.

Punch-in geometry: centre crop to 1500x844 (16:9) rescaled to 1920x1080.
Modest on purpose — clarity holds and it never reads as digital zoom.

## Cutting rhythm follows the emotional shape

| Section | Time | Step | Why |
|---|---|---|---|
| Arrival + water comedy | 0–130 s | 2.8 s | spectacle and gags, fastest |
| Tipping bucket | 130–152 s | 3.0 s | build to the drench |
| Lazy river, Teddy lost, rescue | 152–215 s | **5.5 s** | emotional core, deliberately calmer |
| Slide nerves and climb | 215–265 s | 3.2 s | rising anticipation |
| Ride and splash | 265–290 s | **2.5 s** | peak energy, fastest cutting |
| Towels and sleepy ending | 290–306 s | **7.9 s** | warm ending left alone |

The tender beats keep long takes. Only the spectacle is cut hard.

## Build

Six chunks of ~50 s, each generated by `gen.py` from a segment table and
encoded in a single foreground sandbox call, then joined with the concat
demuxer (stream copy) and muxed with the original Suno MP3. Background mode is
still returning `deadline_exceeded`, so chunking remains necessary.

Verified: 1920x1080, h264 + aac, 24 fps, 305.751 s against a 305.640 s song.

## Not yet applied

The opening test also **dropped the walk-in scene** to reach water faster. V3
keeps it, because removing it shifts every subsequent cue and would break the
audio sync that made this build cheap and safe. Worth doing as a follow-up if
the pacing change proves itself.

Still unverified by eye: nobody has visually inspected the punch-ins for
softness or awkward reframing. The 1.28x factor is conservative, but the
480p preview exists so this can be judged quickly.
